    ...
    async execute(interaction: ChatInputCommandInteraction, _client: ExtendedClient) {
        ...
        if (!interaction.memberPermissions?.has(PermissionFlagsBits.KickMembers)) {
            return interaction.reply({
                embeds: [errorEmbed('صلاحيات غير كافية', 'تحتاج صلاحية "طرد الأعضاء" لتنفيذ هذا الأمر.')],
                ephemeral: true
            });
        }
        ...
        if (!targetMember.kickable) {
            return interaction.reply({
                embeds: [errorEmbed('لا يمكن الطرد', 'لا أستطيع طرد هذا العضو، ربما تكون رتبته أعلى مني.')],
                ephemeral: true
            });
        }
        ...
        if (
            ...
            return interaction.reply({
                embeds: [errorEmbed('غير مسموح', 'لا يمكنك طرد عضو برتبة أعلى منك أو تساويك.')],
                ephemeral: true
            });
        }

...
export function guildBackgroundPath(guildId: string): string {
    return path.join(WELCOME_DIR, `${guildId}.png`);
}

export function hasCustomBackground(guildId: string): boolean {
 ...
 * @param member كائن GuildMember
 * @param opts خيارات التخصيص
 * @returns PNG buffer
 */
export async function generateWelcomeCard(member: GuildMember, opts: WelcomeCardOptions = {}): Promise<Buffer> {
    const W = 1200;
    const H = 420;
    ...
    // ---------- الخلفية ----------
    let bg: import('canvas').Image | null = null;
    try {
    ...
    try {
        avatar = avatarURL ? await loadImage(avatarURL) : null;
    } catch {
        avatar = null;
    }
    ctx.save();
